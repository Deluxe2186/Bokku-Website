...
export default function Navbar() {
  ...
  return (
    <nav className="w-full bg-white/95 backdrop-blur border-b border-gray-100 sticky top-0 z-50 shadow-sm">
      ...
      <div className="bg-[#001DDD] text-white text-xs py-2 px-4 sm:px-6 lg:px-8 flex justify-between items-center hidden sm:flex">
        <div className="font-medium">Get Up to 70% Discount Everyday on Household Essentials!</div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center gap-4">
        {/* Brand Logo */}
        <Link href="/" className="font-display text-2xl font-bold text-[#001DDD] tracking-tight flex-shrink-0">
          bokku<span className="text-orange-500">!</span><span className="text-gray-400 font-medium text-lg">Mart</span>
        </Link>
      ...
      <div className="border-t border-gray-100 hidden md:block bg-[#FAF8F3]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex gap-8 text-sm font-medium text-gray-600">
          ...
          <Link href="/" className="hover:text-[#001DDD]">Fresh Produce</Link>
          <Link href="/" className="hover:text-[#001DDD]">Bokku Bakery</Link>
          <Link href="/" className="hover:text-[#001DDD]">Household</Link>
          <Link href="/" className="hover:text-[#001DDD]">Health & Beauty</Link>
        </div>
      </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 space-y-3 font-medium shadow-inner">
          ...
          <Link href="/" className="block text-gray-700 py-1 border-b border-gray-50">Fresh Produce</Link>
          <Link href="/" className="block text-gray-700 py-1 border-b border-gray-50">Bokku Bakery</Link>
          <Link href="/" className="block text-gray-700 py-1 border-b border-gray-50">Household</Link>
          <Link href="/" className="block text-gray-700 py-1 border-b border-gray-50">Health & Beauty</Link>
        </div>
      )}
    </nav>
  );
